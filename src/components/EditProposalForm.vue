<template>
  ...
  const handleSubmit = async () => {
    try {
      ...
      const proposalResponse = await fetch('http://localhost:3000/v1/proposals', {
        method: 'POST',
        headers: { 
      ...
      const newProposalId = proposalResult.proposal.id;
      ...
      window.location.href = `/propuestas?id=${roomId.value}`;
    } catch (err) {
